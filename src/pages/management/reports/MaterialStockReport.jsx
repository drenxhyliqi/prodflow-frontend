...
export default function MaterialsStockReport() {
    const { checking, denied, startDate, endDate, withRunId, runId } = useReportAccess('materials_stock')
    ...
    const loadReport = useCallback(async () => {
        ...
        try {
            const response = await api.get(
                `/admin/materials_stock_report?${withRunId(`start_date=${startDate}&end_date=${endDate}`)}`
            )
            ...
            <div className="d-flex flex-wrap justify-content-between align-items-start gap-3 mb-4">
                <div>
                    ...
                    <small style={{ color: '#6b7280' }}>
                        {startDate && endDate ? `${startDate} — ${endDate}` : 'Inventory analytics · Stock movements'}
                    </small>
                </div>
                <button
                    type="button"
