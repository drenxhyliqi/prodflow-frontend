...
const Contracts = () => {
    ...
    if (editContract) return (
        <Layout>
            <button onClick={() => setEditContract(null)} className='btn border-0 p-0 d-flex align-items-center fw-semibold mb-3'>
                    ...
                    <form onSubmit={handleUpdate}>
                        <div className="row g-3">
                            <div className="col-md-3">
                                ...
                                <select name="employee_id" defaultValue={editContract.employee_id} className={inputCls} required>
                                    {staff.map(s => <option key={s.sid} value={s.sid}>{s.name} {s.surname}</option>)}
                                </select>
                            </div>
                            <div className="col-md-3">
                                <label className="form-label small fw-semibold">Start Date</label>
                                <input type="date" name="start_date" defaultValue={editContract.start_date} className={inputCls} required />
                            </div>
                            <div className="col-md-3">
                                <label className="form-label small fw-semibold">End Date</label>
                                <input type="date" name="end_date" defaultValue={editContract.end_date} className={inputCls} />
                            </div>
                            <div className="col-md-3">
                                <label className="form-label small fw-semibold">Status</label>
                                <select name="status" defaultValue={editContract.status} className={inputCls}>
            ...
            <div className="card rounded-4 border-0 shadow-sm">
                <div className="card-body p-4">
                    <div className="d-flex justify-content-between align-items-center mb-4">
                        <h6 className='fw-bold mb-0'>Contracts List ({pagination.total || 0})</h6>
                                    ...
                                    <tr key={c.cid}>
                                        ...
                                        <td>
                                            <div className="d-flex align-items-center gap-2">
                                                ...
                                                <span className='fw-semibold'>{c.employee_name} {c.employee_surname}</span>
                                            </div>
                                        </td>
                                        ...
                                        <td>
                                            <span style={{
                                                ...
                                                background: c.status?.toLowerCase() === 'active' ? '#10b98120' : '#ef444420',
                                                color: c.status?.toLowerCase() === 'active' ? '#10b981' : '#ef4444'
                                            }}>
                                                {c.status}
                                            </span>
                                        </td>
                                        <td className="text-end">
                                            <button onClick={() => setEditContract(c)} className="btn btn-sm me-1" style={{ color: BRAND, background: BRAND + '12' }}><FaEdit /></button>
